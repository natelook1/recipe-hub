...
const DENSITY_G_PER_CUP: Record<string, number> = {
  ...
  'ground almonds': 96, 'shredded coconut': 75, 'desiccated coconut': 75,
...
export class GeminiService {
  ...
  private toolConvertUnit(amount: number, fromUnit: string, toUnit: string) {
  ...
  private toolConvertByIngredient(amount: number, fromUnit: string, toUnit: string, ingredientName: string) {
    const f = fromUnit.toLowerCase().trim();
    const t = toUnit.toLowerCase().trim();
    const ing = ingredientName.toLowerCase().trim();
    const gPerCup = DENSITY_G_PER_CUP[ing];
    if (!gPerCup) return { error: `Unknown ingredient "${ingredientName}" — use convert_unit for same-type conversion instead` };
    const gPerMl = gPerCup / 236.588;
    if (UNIT_TO_G[f] && UNIT_TO_ML[t]) {
      const grams = amount * UNIT_TO_G[f];
      return { amount: Math.round((grams / gPerMl / UNIT_TO_ML[t]) * 10000) / 10000, unit: toUnit };
    }
    if (UNIT_TO_ML[f] && UNIT_TO_G[t]) {
      const ml = amount * UNIT_TO_ML[f];
      return { amount: Math.round((ml * gPerMl / UNIT_TO_G[t]) * 10000) / 10000, unit: toUnit };
    }
    return this.toolConvertUnit(amount, fromUnit, toUnit);
  }

  private buildSystemPrompt(preferredSystem: string) {
...
`.trim();
  ...
  private async withRetry<T>(fn: () => Promise<T>, { retries = 3, baseDelayMs = 1500 } = {}): Promise<T> {
    let lastErr: any;
  ...
  private async runWithTools(model: any, contents: any[]) {
    const MAX_TURNS = 20;
    let turn = 0;
    while (turn++ < MAX_TURNS) {
      const result = await this.withRetry(() => model.generateContent({ contents, tools: TOOLS })) as any;
      ...
      const responseParts = fnCalls.map((part: any) => {
        const { name, args } = part.functionCall;
        let result: any;
        if (name === 'convert_unit') {
          result = this.toolConvertUnit(args.amount, args.from_unit, args.to_unit);
        } else if (name === 'convert_by_ingredient') {
          result = this.toolConvertByIngredient(args.amount, args.from_unit, args.to_unit, args.ingredient_name);
        } else {
  ...
  private extractJsonLd(html: string): string | null {
  ...
  async extractFromUrl(url: string, preferredSystem = 'metric') {
    ...
    const sourceImageUrl = this.extractOgImage(html);
    const text = this.htmlToText(html).slice(0, 12000);
    const userContent = jsonLd
      ? `Structured data (prefer this):\n${jsonLd.slice(0, 4000)}\n\nPage text (supplement only):\n${text}`
      : `Page text:\n${text}`;
    const contents = [
      { role: 'user', parts: [{ text: this.buildSystemPrompt(preferredSystem) + '\n\n' + userContent }] },
    ];
    const draft = await this.runWithTools(model, contents);
    if (sourceImageUrl) draft.source_image_url = sourceImageUrl;
    return draft;
  }

  async extractFromText(text: string, preferredSystem = 'metric') {
    console.log(`[Gemini] extractFromText: ${text.length} chars (${preferredSystem})`);
    const model = this.genai.getGenerativeModel({ model: 'gemini-2.5-flash' });
    const contents = [
      { role: 'user', parts: [{ text: this.buildSystemPrompt(preferredSystem) + '\n\nRecipe text:\n' + text.slice(0, 12000) }] },
    ];
    return this.runWithTools(model, contents);
  }

  async extractFromPhoto(imageBuffer: Buffer, mimeType: string, preferredSystem = 'metric') {
    console.log(`[Gemini] extractFromPhoto: ${mimeType}, ${imageBuffer.length} bytes (${preferredSystem})`);
    const model = this.genai.getGenerativeModel({ model: 'gemini-2.5-flash' });
    const contents = [{
      role: 'user',
      parts: [
        { text: this.buildSystemPrompt(preferredSystem) + '\n\nExtract the recipe from this image.' },
